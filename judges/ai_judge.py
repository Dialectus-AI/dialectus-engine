...
class AIJudge(BaseJudge):
    ...
    async def evaluate_debate(self, context: DebateContext) -> JudgeDecision:
        ...
        try:
            ...
            logger.info(f"AI judge decision: {decision.winner_id} wins by {decision.winner_margin:.1f}")
            return decision
    ...
    def _get_participant_labels(self, participants: List[str], context: DebateContext) -> Dict[str, str]:
        ...
        try:
            ...
            logger.warning(f"Failed to get format-specific labels: {e}, falling back to generic labels")
            # Fallback to generic labels
            fallback_labels = {}
            for i, participant in enumerate(participants):
                fallback_labels[participant] = f"Participant {chr(65 + i)}"  # A, B, C, etc.
            return fallback_labels
    
    def _map_enhanced_label_to_participant_id(self, enhanced_label: str, participants: List[str], context: DebateContext) -> str:
        ...
        logger.warning(f"Could not map enhanced label '{enhanced_label}' to participant ID, using fallback")
        return participants[0] if participants else "unknown"
    
    async def _generate_evaluation(self, transcript: str, context: DebateContext) -> str:
        ...
        evaluation_prompt = self._create_evaluation_prompt(transcript, participants, criteria_list, context)
...
You must respond with a structured JSON evaluation that can be parsed programmatically. Be thorough but concise in your reasoning."""
    ...
    def _create_evaluation_prompt(self, transcript: str, participants: List[str], criteria: List[str], context: DebateContext) -> str:
...
Provide your evaluation as valid JSON only, no additional text:"""
    ...
    def _parse_evaluation(self, evaluation: str, context: DebateContext) -> JudgeDecision:
        ...
        try:
            ...
            winner_id = self._map_enhanced_label_to_participant_id(winner_enhanced_label, participants, context)
            ...
            for score_data in evaluation_data.get("criterion_scores", []):
                ...
                participant_id = self._map_enhanced_label_to_participant_id(participant_enhanced_label, participants, context)
                
                # Debug and fix feedback field too
                feedback = score_data.get("feedback", "")
                if not isinstance(feedback, str):
                    logger.warning(f"Converting non-string feedback: {type(feedback)} -> {feedback}")
            ...
            logger.debug(f"Judge overall_feedback type: {type(overall_feedback)}, value: {overall_feedback}")
            ...
            if not isinstance(overall_feedback, str):
                logger.warning(f"Converting non-string overall_feedback: {type(overall_feedback)} -> {overall_feedback}")
                overall_feedback = str(overall_feedback) if overall_feedback else ""
                
            if not isinstance(reasoning, str):
                logger.warning(f"Converting non-string reasoning: {type(reasoning)} -> {reasoning}")
    ...
    def _create_fallback_decision(self, context: DebateContext) -> JudgeDecision:
        ...
        participant_scores = {}
        for participant in participants:
            messages = [msg for msg in context.messages if msg.speaker_id == participant]
            total_length = sum(len(msg.content) for msg in messages)
            participant_scores[participant] = total_length
        
        # Determine winner (longest total response)
        if participant_scores:
            winner_id = max(participant_scores.keys(), key=lambda x: participant_scores[x])
        else:
...
class EnsembleJudge(BaseJudge):
    ...
    def _combine_decisions(self, decisions: List[JudgeDecision], context: DebateContext) -> JudgeDecision:
        ...
        ensemble_winner = max(winner_votes.keys(), key=lambda x: winner_votes[x]) if winner_votes else "unknown"
        ...
        for criterion in self.criteria:
            for participant in participants:
                ...
                if participant_scores:
                    ...
                    feedbacks = [
                        s.feedback for s in all_scores 
                        if s.criterion == criterion and s.participant_id == participant
                    ]
                    combined_feedback = " | ".join(feedbacks[:2])  # Limit feedback length
                    
                    averaged_scores.append(CriterionScore(
                        criterion=criterion,
                        participant_id=participant,
                        score=avg_score,
