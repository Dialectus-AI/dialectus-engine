...
class ModelCacheManager:
    ...
    def __init__(self, cache_dir: Path | None = None, default_ttl_hours: int = 6):
        ...
        logger.info(
            f"Model cache manager initialized. Cache dir: {self.cache_dir}, Default TTL: {default_ttl_hours}h"
        )
    ...
    def set(
        ...
        logger.info(
            f"Cached {provider}/{endpoint} response (expires: {expires_at.strftime('%Y-%m-%d %H:%M:%S')})"
        )
